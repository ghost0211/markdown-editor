...
describe('Release Acceptance & Quality Assurance (v1.4.6)', () => {
  describe('Consistent Release Version 1.4.6 across all Project Manifests', () => {
    const rootDir = resolve(__dirname, '..');

    it('package.json should have version 1.4.6', () => {
      const pkg = JSON.parse(readFileSync(resolve(rootDir, 'package.json'), 'utf-8'));
      expect(pkg.version).toBe('1.4.6');
    });

    it('package-lock.json should have root version 1.4.6', () => {
      const lock = JSON.parse(readFileSync(resolve(rootDir, 'package-lock.json'), 'utf-8'));
      expect(lock.version).toBe('1.4.6');
      expect(lock.packages[''].version).toBe('1.4.6');
    });

    it('Cargo.toml should have version 1.4.6', () => {
      ...
      expect(match![1]).toBe('1.4.6');
    });

    it('Cargo.lock should have markdown-editor package version 1.4.6', () => {
